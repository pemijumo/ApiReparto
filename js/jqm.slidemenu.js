...
$(document).on("pageinit", function(e){
	...
	$(document).on("click", ".paginaActiva", function(e) { // ocultar meenu cuando se hace tap en una pagina
		slidemenu2(sm, sm.data('slideopen'));
		e.stopImmediatePropagation();
		e.preventDefault();
	});
	...
	$(window).on('resize', function(e){
		if (sm.data('slideopen')) {
			...
			// $(":jqmData(role='page')").css('left', '150px');
...
function slidemenu2(sm, only_close) {///metodo para ocultar 
	sm.height(viewport().height);
...
function slidemenu(sm, only_close) {
	...
	if (!sm.data('slideopen') && !only_close) {
		sm.show().animate({width: deslizamiento, avoidTransforms: false, useTranslate3d: true}, 'fast');
		$(".ui-page-active").css('left', deslizamiento);
		sm.data('slideopen', true);
		if ($(".ui-page-active :jqmData(role='header')").data('position') == 'fixed') {
			$(".ui-page-active :jqmData(slidemenu)").css('margin-left', deslizamiento);//mover icono de menu
		} else {
